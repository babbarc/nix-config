...
in
{
  # Hermes Agent engine for the wsl host (imported only by
  # hosts/wsl/configuration.nix, alongside modules/dev/joy-brain.nix - never
  # the shared modules/dev list, which the laptop/server hosts also import).
  #
  ...
  # modules/dev/joy-brain.nix - keep those two paths in sync.
  home.packages = with pkgs; [
    uv      # drives the pinned install below (and later `hermes update`)
    ffmpeg  # runtime dep of several hermes tools/skills
    # ripgrep + git + nodejs are already declared by the shared modules/dev
    # (cli-tools.nix / dev-toolchains.nix), so they are not repeated here.
  ];
